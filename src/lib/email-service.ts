...
function getEmailText(status: string, businessName: string, feedback?: string) {
  if (status === "approved") {
    return `
      ...
      You can now access all features of the ecoFundme platform.
      
      Thank you for choosing ecoFundme.
  ...
  if (status === "approved") {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>You can now access all features of the ecoFundme platform.</p>
        <p>Thank you for choosing ecoFundme.</p>
        <p>Best regards,<br>The ecoFundme Team</p>
      </div>
    `
  } else {
    return `
